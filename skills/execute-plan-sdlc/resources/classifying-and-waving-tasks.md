...
|---|---|---|
| Trivial | `gemini-3.7-flash-medium` | Capable baseline; avoids syntax errors on simple tasks. Single trivial → execute inline. Two or more trivials in the same phase → dispatch as one batch agent. |
| Standard | `gemini-3.7-flash-medium` | Capable, cost-efficient |
...
- A prior `gemini-3.7-flash-medium` attempt on a similar task in this project failed

Assign `gemini-3.7-flash-medium` to a Complex task when:
...
|---|---|---|---|---|
| **Speed** | gemini-3.7-flash-low | gemini-3.7-flash-medium | gemini-3.7-flash-high | Plan is well-specified, changes are mechanical |
| **Balanced** | gemini-3.7-flash-medium | gemini-3.7-flash-high | gemini-3.1-pro-low | Default — matches complexity to capability |
| **Quality** | gemini-3.7-flash-medium | gemini-3.1-pro-low | gemini-3.1-pro-high | Codebase is unfamiliar, tasks are ambiguous |
...
When dispatching a wave-runner Agent, permanently lock the orchestrator to `gemini-3.7-flash-low` regardless of the preset. The presets above apply strictly to the *per-task worker subagents* that the wave-runner spawns.
...
8. **Identify in-wave trivial batches:** Within each wave, if 2 or more tasks are classified Trivial, dispatch them together as a single batch agent (using the assigned model for Trivial tasks, e.g., gemini-3.7-flash-medium) rather than executing each inline. A single trivial task in a wave is still executed inline. Same-file ordering rules apply within the batch (see Batched Trivial Tasks Prompt Template below).
...
- **Always run verification commands (tests, builds, linters) and package manager commands (e.g., npm, pnpm, pnpm build, yarn) via the truncated wrapper script** to prevent context bloat: `<PLUGIN_ROOT>/skills/execute-plan-sdlc/scripts/run_truncated.sh "<command>"`
...
## Execution Context
- Assigned model: {MODEL — gemini-3.7-flash-low, gemini-3.7-flash-medium, gemini-3.7-flash-high, gemini-3.1-pro-low, or gemini-3.1-pro-high}
