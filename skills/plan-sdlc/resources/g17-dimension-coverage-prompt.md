...
**Model:** gemini-3.7-flash-medium (from `g17Dispatch.model` in prepare output — do NOT hardcode)
...
Task tool (general-purpose):
  ...
  prompt: |
    ...
    **This gate is advisory (non-blocking).** Never fail or halt plan finalization.
    ...
    Suppression rule: do NOT fire UPDATE criteria for doc-only diffs where ALL plan paths match `docs/**`, `README*`, or `*.md` files outside the `plugins/lift-sdlc/skills/` directory.
