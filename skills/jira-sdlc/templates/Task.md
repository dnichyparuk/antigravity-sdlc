## Objective

[Bullet list:
- Technical goal in one line — what this task delivers
- Why it matters to the system, team, or product
- Dependency or blocker it unblocks (if any)]

## Details

[Bullet list of work to be done:
- Systems, components, or configurations affected
- Current state vs. desired state
- Technical approach or constraints
- Dependencies on other tasks or external systems
- Migration or rollback considerations if applicable]

## Acceptance Criteria

- [ ] [Primary deliverable — system/component behaves as described]
- [ ] [Secondary criterion — dependent behavior works correctly]
- [ ] [No regression — existing functionality unaffected]
- [ ] [Observability — logging/metrics in place if applicable]

## Development Plan

_To be defined by the development team._

## How to Test

1. **Prerequisites:** [Environment, access, tools needed]
2. **Steps:**
   1. [Step 1]
   2. [Step 2]
3. **Expected result:** [Measurable outcome]
4. **Verification:**
   - [How to confirm the change is working correctly]
