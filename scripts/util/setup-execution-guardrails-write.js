#!/usr/bin/env node
/**
 * setup-execution-guardrails-write.js
 * Writes the selected execution guardrails to `.sdlc/config.json` via
 * `scripts/lib/config.js`'s `writeSection`. Replaces the fragile inline
 * `node -e` block in `setup-execution-guardrails_write.sh` (issue: legacy
 * script migration, task 10), which substituted a `<GUARDRAILS_JSON>`
 * placeholder token into a shell-quoted script string. This CLI takes the
 * guardrails array as a structured `--value` flag instead.
 *
 * Usage:
 *   node setup-execution-guardrails-write.js --section <name> --value '<json-array>'
 *
 * Options:
 *   --section <name>   Config section to write (default: "execute")
...
function main(argv) {
  ...
  if (errors.length > 0) {
    writeOutput({ errors, warnings: [] }, 'setup-execution-guardrails-write', 1);
    return;
  }
  ...
  writeOutput(
    ...
    'setup-execution-guardrails-write',
    0
  );
}
...
if (require.main === module) {
  try {
    ...
    process.stderr.write(`setup-execution-guardrails-write.js error: ${err.message}\n${err.stack}\n`);
    process.exit(2);
  }
}
