...
This document describes Lift-SDLC's built-in state tracking and session management system.
...
|-------|-------------|-------------|
| gemini-3.7-flash (all variants) | 1M tokens | ~3M bytes |
...
```json
{
  "permissions": {
    "allow": [
      ...
      "command(.*/\.gemini/config/plugins/lift-sdlc/skills/.*/scripts/.*)"
    ]
  }
}
```
